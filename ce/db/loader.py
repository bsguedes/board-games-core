...
from ce.champions.champion import Channel, Champion, ChampionLevel
...
class Loader:
    ...
    def __init__(self):
        with open('ce/db/v1.json', 'r') as f:
            data = json.load(f)
        self.Abilities = {
        ...
        self.Cards = [
            Card(c['ID'], c['TopRow'], c['MidRow'], c['BottomRow'], c['Cash'], c['Attribute'], c['Points'], c['CE'],
        ...
        self.Channels = [
            Channel(channel, [
                Champion(b['Champion'], [
                    ChampionLevel(b['Level'], b['AbilityName'], b['Points'], b['CashCost'],
                                  b['CardCost'], b['TalentCost'], self.Abilities[b['AbilityID']])]) for b in level_list
            ]) for channel, level_list in group_by(data['Champions'], lambda c: c['Channel']).items()
        ]
